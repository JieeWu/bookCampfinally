...
export default function CartPartStep({ icon, name, isActive }) {
  ...
  return (
    <>
      <div
        className={`${styles.stepDiv} ${styles.step} ${
          isActive ? styles.stepOK : ''
        }`}
      >
        <div className={styles.stepContent}>
          {/* <i
            className={`${styles.stepFinger} fa-regular fa-hand-point-right`}
          ></i> */}
          {additionalImg}
          <i className={icon}></i>
      ...
      {name === '訂單完成' ? (
        ...
        <div
          className={`${styles['cart-part-line-margin']} ${
            isActive ? 'text-orange' : ''
          }`}
        >
          {/* <span className={`${styles["cart-part-line"]}`}></span> */}
          {/* <span className={`${styles["cart-part-line"]}`}></span> */}
          <i
            className={`fa-solid fa-caret-right ${isActive ? 'arrow-one' : ''}`}
          ></i>
          <i
            className={`fa-solid fa-caret-right ${isActive ? 'arrow-two' : ''}`}
          ></i>
        </div>
      )}
    </>
