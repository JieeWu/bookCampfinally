...
export default function Cart() {
  ...
  const { cartItem, setCartItem,
    orderCH, setOrderCH } = useContext(CartContext);
  // 使用會員
  ...
  if (cartCoupon.length > 0) {
    let operator = coupon[parseInt(cartCoupon) - 1];
    ...
    if (operator.discount_type == 0) {
      ...
      amountCoupon = Math.round(amount * parseFloat(operator.discount));
    }
  } else {
    amountCoupon = amount;
  }
        ...
        <div className='row' style={cartSpacing}>
          ...
          <div className='col-12 col-xxl-8 d-flex flex-column'>
            ...
            <div className={`frame-line-black ${styles.cart_bg}`}>
              {cartItem.map((item) => {
                return (
                  <CartBook item={item} selectCheck={selectCheck} check={check} />
                )
              })}
            </div>
            {/* 優惠券&點數區塊 */}
            <div className='d-flex flex-wrap mt-4 '>
              <CartCoupon setCartCoupon={setCartCoupon} />
              <CartPoint
                amount={amount}
                point={point}
                usePoint={usePoint}
          ...
          {/* 描述區 */}
          <CartDescribe seeAddItem={seeAddItem} amount={amount} check={check} couponName={couponName} usePoint={usePoint} total={total} cartCoupon={cartCoupon} />
        </div>
      }
      {/* 加購彈跳視窗 */}
