...
export default function LineLoginJWT() {
  ...
  const lineLogout = async () => {
    ...
    const res = await axios.get(
      `http://localhost:3002/api/line-login/logout?line_uid=${line_uid}`,
      {
  ...
  const goLineLogin = () => {
    ...
    axios
      .get('http://localhost:3002/api/line-login/login', {
  ...
  useEffect(() => {
    ...
    if (router.isReady) {
      ...
      const cbUrl = `http://localhost:3002/api/line-login/callback?${qs}`
