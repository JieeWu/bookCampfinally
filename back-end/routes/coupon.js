...
router.post('/received', async (req, res) => {
  ...
  try {
    ...
    SELECT CR.*, C.*,
      ...
      BI.book_img_url, B.book_id, CG.b_genre_id
    ...
    LEFT JOIN book_img BI ON BI.book_img_id = B.book_img_id
...
router.post('/coupon_code/search', async (req, res) => {
  ...
  try {
    ...
    const sql =
      type === true
        ? "SELECT A.*, DATE_FORMAT(A.start_time, '%Y-%m-%d') start_time, DATE_FORMAT(A.end_time, '%Y-%m-%d') end_time FROM `coupon` A WHERE A.coupon_code = ? AND A.coupon_status = 1;"
...
router.get('/frontCoupon', authenticate, async (req, res) => {
  ...
  const sql =
    'SELECT * FROM `coupon` as c WHERE NOT EXISTS (SELECT 1 FROM `coupon_record` cr WHERE cr.client_id = ? AND cr.coupon_id = c.coupon_id) LIMIT 4;'
