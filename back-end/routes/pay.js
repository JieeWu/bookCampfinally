...
router.post("/linepay", async (req, res) => {
    try {
        // 接收前端資料
        const order = req.body.orderCH;
        const product = req.body.cartItem;

        let user, phone, address, total, cid, coupon, delivery, pay, receipt, fee, other;
        user = order.consignee;
        phone = order.consigneePhone;
        address = order.consigneeAddress;
        total = order.usertotal;
        cid = order.id;
        coupon = order.coupon;
...
router.get("/linepay/confirm", async (req, res) => {
    try {
        ...
        const url = `${LINEPAY_SITE}/${LINEPAY_VERSION}${uri}`;
        const linePayRes = await axios.post(url, resOrder, { headers })

        // // 刪除購物車的資料
        const sql = `DELETE FROM cart WHERE paymentCode = ?`;
        await db.query(sql, [orderId]);

        res.redirect('http://localhost:3000/cart/checkout3');
    }
    catch (error) {
        console.log(error);
    }
