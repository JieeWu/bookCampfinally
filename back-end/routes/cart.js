...
router.post("/finish", authenticate, async (req, res) => {
  try {
    ...
    // 訂單資料
    const ordersql = `SELECT consignee, consignee_phone, consignee_address, total, delivery_fee, other
    FROM \`order\`
    ...
    // 優惠卷
    const couponsql = `SELECT coupon_name FROM coupon WHERE coupon_id = ${cid}`;
